...
export const asyncFileReaderAsDataURL = (file: File | Blob): Promise<string> => {
  // 添加类型检查和验证
  if (!file || !(file instanceof Blob)) {
    return Promise.reject(new Error('Invalid file parameter: expected Blob or File object'));
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = (e) => {
      reject(new Error(`File reading failed: ${e}`));
    };
